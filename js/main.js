...
document.addEventListener("DOMContentLoaded", () => {
    ...
    [mouseMoveParallaxGallery, barbaPageTransition, smoothScroller, mouseMoveParallaxLine, initProjectCaseAnimation, animateNumbering, expertiseHover, horizontalScrollPlayground, parallaxImg].forEach(func => func());
});
...
function barbaPageTransition() {
    ...
    barba.hooks.after(() => document.querySelector('html').classList.remove('is-transitioning'));
...
function animateProjectCase(projectCase, entering) {
    ...
    gsap.to(projectCase.querySelector(".main_img"), { scale: entering ? 1 : 1.5, duration: 0.5 });
}
...
function horizontalScrollPlayground() {
    ...
    gsap.to(horizontalScroll, {
        ...
        ease: 'none',
        scrollTrigger: {
            trigger: '.playground', pin: true, scrub: true, start: 'top top', end: () => `+=${horizontalScrollLength}`, invalidateOnRefresh: true
        }
    });
}
...
function mouseMoveParallaxGallery() {
    ...
    frames.forEach(frame => {
        frame.addEventListener('mouseenter', () => {
            gsap.to(frame, { scale: 1.5, duration: 0.5, zIndex: 2, ease: "power1.out" });
        });
        frame.addEventListener('mouseleave', () => {
            gsap.to(frame, { scale: 1, duration: 0.5, zIndex: 1, ease: "power1.out" });
        });
    });
}
